...
export const StrategicPillars = ({ isMobile }: { isMobile: boolean }) => {
  return (
    <section style={{ 
        position: 'relative', 
        zIndex: 1, 
        // Aumentamos el padding para que las ondas no tapen el contenido
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', position: 'relative', zIndex: 10 }}>
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} style={{ textAlign: 'center', marginBottom: '60px' }}>
              {/* Título adaptado para fondo blanco */}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(3, 1fr)', gap: '30px' }}>
              {[
                  ...
                  <motion.div 
                      ...
                      whileHover={{ 
                          y: -8, 
                          backgroundColor: '#001848', // Un azul ligerísimamente más claro al pasar el mouse
                          boxShadow: '0 20px 40px rgba(0, 194, 255, 0.2)' 
                      }}
                      style={{ 
                        padding: '40px 30px', 
                        backgroundColor: '#000c2d', // Cuadros Azules
                        borderRadius: '16px', 
                        border: `1px solid rgba(0, 194, 255, 0.2)`, // Borde cian sutil
                        transition: 'all 0.3s ease',
                        boxShadow: '0 10px 30px rgba(0,0,0,0.1)'
                      }}
                  >
                      <div style={{ marginBottom: '20px' }}>{val.icon}</div>
                      {/* Textos en blanco/gris claro para que contrasten sobre el cuadro azul */}
      ...
      <style>{`
        /* Estilos de las ondas adaptados para este componente */
        .wave-container { 
            position: absolute; 
            left: 0; 
            width: 100%; 
            height: clamp(100px, 12vw, 180px); /* Se adapta en móvil */
            overflow: hidden; 
            line-height: 0; 
            z-index: 1; 
            pointer-events: none; 
        ...
        /* Animaciones para que las 3 capas se muevan a distintos ritmos */
